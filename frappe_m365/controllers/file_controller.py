...
def get_file_path(doc):
    try:
        folder_path = "/private/files/" if doc.is_private else "/public/files/"
        filepath = os.path.abspath(os.curdir) + "/" + \
            frappe.utils.cstr(frappe.local.site) + \
            folder_path + doc.file_name
        return filepath
    except Exception as e:
        frappe.log_error("file path error", e)
        return None
...
def get_group_doc(doctype, group_settings):
    try:
        ...
        frappe.log_error("group doc error", e)
        return None, None
...
def get_group_members(group):
    try:
        ...
        frappe.log_error("group members error", e)
        return []
